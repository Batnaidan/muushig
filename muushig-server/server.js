...
io.on('connection', function (socket) {
  ...
  socket.on('changePlayerReadyState', (playerId) => {
    io.to(clientRoom).emit('playerStateChange', playerId); //change Player ready state
  });
  socket.on('shuffleCards', (roomId) => {
    io.to(clientRoom).emit('deck', shuffleCards());
  });
